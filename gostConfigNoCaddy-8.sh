...
mv gost-fullua-8-small-linux gost-8
chmod +x gost-8
...
nohup ./gost-8 -L wss://$your_userid:$your_pass@0.0.0.0:20?compression=true >/dev/null 2>./log/gost20.log& 

nohup ./gost-8 -L wss://$your_userid:$your_pass@0.0.0.0:1433?compression=true >/dev/null 2>./log/gost1433.log&

nohup ./gost-8 -L wss://$your_userid:$your_pass@0.0.0.0:3306?compression=true >/dev/null 2>./log/gost3306.log&

nohup ./gost-8 -L wss://$your_userid:$your_pass@0.0.0.0:69?compression=true >/dev/null 2>./log/gost69.log& 

nohup ./gost-8 -L wss://$your_userid:$your_pass@0.0.0.0:3389?compression=true >/dev/null 2>./log/gost3389.log&

nohup ./gost-8 -L wss://$your_userid:$your_pass@0.0.0.0:1521?compression=true >/dev/null 2>./log/gost1521.log&
...
wget https://github.com/dannywei7/vgost/raw/master/gostClient-8.zip
unzip gostClient-8.zip
mv -f gostClient-8 /usr/local/bin/gostClient-8

cat > /usr/local/bin/gostClient-8/peer1.txt  <<-EOF
strategy        random
max_fails       1
fail_timeout    5s
...
peer    wss://$your_userid:$your_pass@localhost:1080?ip=$your_domain:1521&compression=true

#peer    https://$your_userid:$your_pass@localhost:1080?ip=$your_domain:8080

EOF

cd /usr/local/bin/gostClient-8
zip -q -r gost-cli-8.zip /usr/local/bin/gostClient-8/
mkdir -p /var/www/html/client
mv -f /usr/local/bin/gostClient-8/gost-cli-8.zip /var/www/html/client/
	...
	yellow "http://${your_domain}/client/gost-cli-8.zip"	
